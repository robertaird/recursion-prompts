...
// var factorial = num => num === 0 ? 1 : num * factorial(--num); // this did not check for null

// I wonder if this is too difficult to read..
var factorial = num => num < 0 ? null : num === 0 ? 1 : num * factorial(--num);
...
var arraySum = (array, inner = []) => Array.isArray(array[0]) ? arraySum(array[0]) + arraySum(array.slice(1)) : array.length === 0 ? 0 : array[0] + arraySum(array.slice(1));

// var arraySum = (array, inner = []) => {
...
var reverse = string => string === '' ? '' : reverse(string.substr(1)) + string.charAt(0);
...
var createArray = str => str === '' ? [] : [str[0]].concat(createArray(str.slice(1)));
// This will also work with [].concat(str[0], createArray(str.slice(1)))...

// 17. Reverse the order of an array
var reverseArr = array => array.length === 0 ? [] : [array[array.length - 1]].concat(reverseArr(array.slice(0, array.length - 1)));

// 18. Create a new array with a given value and length.
// buildList(0,5) // [0,0,0,0,0]
// buildList(7,3) // [7,7,7]
var buildList = (value, length) => length === 0 ? [] : [value].concat(buildList(value, --length));

// 19. Count the occurence of a value inside a list.
// countOccurrence([2,7,4,4,1,4], 4) // 3
// countOccurrence([2,'banana',4,4,1,'banana'], 'banana') // 2
var countOccurrence = (array, value) => array.length === 0 ? 0 : (array[0] === value ? 1 : 0) + countOccurrence(array.slice(1), value); 
// {
//     if (array.length === 0) {
//         return 0;
//     }
...
var rMap = (array, callback) => array.length === 0 ? [] : [callback(array[0])].concat(rMap(array.slice(1), callback));
...
var capitalizeFirst = array => array.length === 0 ? [] : [array[0].charAt(0).toUpperCase() + array[0].slice(1)].concat(capitalizeFirst(array.slice(1)));
...
var compress = list => list.length === 0 ? [] : [].concat(list[0] === list [1] ? [] : [list[0]], compress(list.slice(1)));
...
var augmentElements = (array, aug) => array.length === 0 ? [] : [array[0].concat(aug)].concat(augmentElements(array.slice(1), aug));

// 33. Reduce a series of zeroes to a single 0.
// minimizeZeroes([2,0,0,0,1,4]) // [2,0,1,4]
// minimizeZeroes([2,0,0,0,1,0,0,4]) // [2,0,1,0,4]
var minimizeZeroes = array => array[0] === 0 && array[1] === 0 ? (array.length === 0 ? [] : [].concat(minimizeZeroes(array.slice(1)))) : (array.length === 0 ? [] : [array[0]].concat(minimizeZeroes(array.slice(1))));
...
// alternateSign([-2,-7,8,3,-1,4]) // [2,-7,8,-3,1,-4]
// This could probably be shortened in the same manner as I did minimizeZeroes, but again, readability is a concern.
...
var numToText = str => {
    ...
    let numDigits = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'];
...
var tagCount = (tag, node) => {
    let count = 0;
    let nodes = node ? node.childNodes : document.childNodes;
    nodes.forEach(node => {
...
var binarySearch = (array, target, min, max) => {
    ...
    firstHalf = array.slice(0, halfway);
    secondHalf = array.slice(halfway);
...
var mergeSort = array => {
    ...
    firstHalf = mergeSort(array.slice(0, halfway));
    secondHalf = mergeSort(array.slice(halfway));
